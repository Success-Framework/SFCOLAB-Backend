...
async function main() {
  ...
  if (Array.isArray(data.users)) {
    for (const u of data.users) {
      const update = {
        ...
        profile: u.profile || { picture: null, bio: null, company: null, socialLinks: {} },
  ...
  if (Array.isArray(data.refreshTokens)) {
    for (const rt of data.refreshTokens) {
      ...
      await RefreshToken.create({ userId, token: rt.token, createdAt: rt.createdAt ? new Date(rt.createdAt) : new Date() });
    }
  ...
  if (Array.isArray(data.knowledgeResources)) {
    for (const r of data.knowledgeResources) {
      ...
      const doc = await Knowledge.create({
        ...
        author: { id: authorId, firstName: r.author?.firstName, lastName: r.author?.lastName },
    ...
    console.log(`Knowledge resources migrated: ${data.knowledgeResources.length}`);
  }

  if (Array.isArray(data.knowledgeComments)) {
    for (const c of data.knowledgeComments) {
      const resourceId = idMap.get(c.resourceId) || ensureObjectId(c.resourceId);
      const authorId = idMap.get(c.author?.id) || ensureObjectId(c.author?.id);
      await KnowledgeComment.create({
        resourceId,
        content: c.content,
        author: { id: authorId, firstName: c.author?.firstName, lastName: c.author?.lastName },
        createdAt: c.createdAt ? new Date(c.createdAt) : undefined,
        updatedAt: c.updatedAt ? new Date(c.updatedAt) : undefined,
      });
    }
    console.log(`Knowledge comments migrated: ${data.knowledgeComments.length}`);
  }
  ...
  if (Array.isArray(data.resourceDownloads)) {
    ...
    console.log(`Resource downloads migrated: ${data.resourceDownloads.length}`);
  }
  ...
  if (Array.isArray(data.ideas)) {
    for (const i of data.ideas) {
      const creatorId = idMap.get(i.creator?.id) || ensureObjectId(i.creator?.id);
      const doc = await Idea.create({
        ...
        industry: i.industry,
        stage: i.stage,
        teamMembers: Array.isArray(i.teamMembers) ? i.teamMembers.slice(0, 3) : [],
        tags: Array.isArray(i.tags) ? i.tags : [],
        creator: { id: creatorId, firstName: i.creator?.firstName, lastName: i.creator?.lastName },
  ...
  if (Array.isArray(data.startups)) {
    for (const st of data.startups) {
      const creatorId = idMap.get(st.creator?.id) || ensureObjectId(st.creator?.id);
      const doc = await Startup.create({
        ...
        creator: { id: creatorId, firstName: st.creator?.firstName, lastName: st.creator?.lastName },
        status: 'active',
        members: Array.isArray(st.members) ? st.members.map(m => ({
        ...
        memberCount: st.memberCount || (Array.isArray(st.members) ? st.members.length : 1),
        views: st.views || 0,
